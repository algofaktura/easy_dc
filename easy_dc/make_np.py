...
def make_vertices(ORD: int) -> Verts:
    ...
    for lvl in range(1, ORD_N[ORD]):
        stages[lvl] = {(array(vec) + xyz).data for vec in stages[lvl - 1] for xyz in BV}
...
def make_edges(V: Verts, VI: IdxMap, unit: int = 2) -> Edges:
    ...
    return tuple(((i, VI[e]) for i, n in enumerate((((array(p) + xz).data for xz in BV if VS.issuperset([(array(p) + xz).data])) for p in V)) for e in n))
